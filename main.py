...
def object_recognition(frame, tester, names, colors, counters):
    ...
    for *box, conf, cls in pred.tolist():
        if names[int(cls)] in ['truck', 'other person', 'motorcycle', 'bus', 'other vehicle', 'rider',
                               'pedestrian', 'bicycle', 'train', 'car', 'trailer']:
            if conf > opt.confidence:
                ...
                cv2.putText(frame, "ID: {}".format(id),
                            ...
                            cv2.FONT_HERSHEY_COMPLEX, 0.6, (100, 100, 255), 1)
